 ...
 * Ex.: "Efésios 4:13", "Sl 23", "Gênesis 1", "Romanos 8", "1 Co 13"
 */
export function extractBibleReference(text: string): { book: string; chapter: number } | null {
  ...
  // Ordena livros por comprimento de alias (mais longos primeiro para evitar colisões como 'jo' antes de 'joao')
  ...
  for (const book of sortedBooks) {
    ...
    for (const name of allNames) {
      // Padrão 1: "Livro 4:13", "Livro 4,13", "Livro 4"
      // Padrão 2: "Livro cap. 4", "Livro capitulo 4"
      const escapedName = name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      // Padrões suportados: "João 3", "Gn 1", "Romanos 8", "1Co 13", "Sl 23:1", "Efésios cap. 4", "Mt 5:1-12"
      const regex = new RegExp(
        `(?:^|[^a-z0-9])${escapedName}(?:\\s*cap[ií]tulo|\\s*cap\\.?)?\\s*(\\d{1,3})(?:[:.,\\s]\\d+)?(?:-\\d+)?(?:\\b|$)`,
        'i'
      );
