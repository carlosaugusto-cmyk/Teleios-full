/**
 * security.service.ts
 * Serviço central de segurança da plataforma Teleios.
 * Gerencia hash de senha, JWT, proteção contra brute-force e sanitização de inputs.
 */
...
// (Utilizado só no mockStore para seed inicial sem await)
// ==========================================

/**
 * Hash síncrono simples para seed inicial.
 * Apenas para inicialização — em produção usar hashPassword.
 */
export function simpleHash(input: string): string {
  ...
  // Converter para hex de 32 caracteres
  const hex = Math.abs(hash).toString(16).padStart(8, '0');
  // Expandir para simular hash de 64 chars (não criptográfico — só para seed)
...
function simpleSign(payload: string): string {
  // Assinatura simples XOR com a chave secreta (suficiente para proteção interna)
...
export function sanitizeString(input: string): string {
  return input
    ...
    .replace(/data:/gi, '')
    .trim()
    .slice(0, 500); // Limite máximo de tamanho
}
